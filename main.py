...
def get_results_grid(url, amazon_url, amazon_name, item_name, is_warehouse):
...
def get_results_list(url, amazon_url, amazon_name, item_name, is_warehouse):
    html_elements = requests.get(url, headers=HEADERS).text
    soup = BeautifulSoup(html_elements, "lxml")
    items_div = soup.find("div", class_="s-main-slot s-result-list s-search-results sg-row")
    items = items_div.find_all("div", class_ = "s-result-item s-asin sg-col-0-of-12 sg-col-16-of-20 sg-col s-widget-spacing-small sg-col-12-of-16")
    results_dict = deepcopy(RESULTS)
    for item in items:
        item_title = item.find("div", class_="a-section a-spacing-none puis-padding-right-small s-title-instructions-style").text
        try:
            item_price = item.find("span", class_= "a-price-whole") if not is_warehouse else item.find("div", class_="a-row a-size-base a-color-secondary").find("span", class_="a-color-base")
        except:
            print(item.find("div", class_="a-row a-size-base a-color-secondary"))
            print(amazon_url[:-1] + item.find("a", class_="a-link-normal s-underline-text s-underline-link-text s-link-style a-text-normal")["href"])
 

        if not item_price:
            #print(item_price)
            print("The following listing did not have a specified price:", item_title)
            continue
        
        #print(item_price)
        if is_warehouse:
            item_price = int(item_price.text[:-5].replace("\u202f", "").replace(".", "").replace(",", ""))
        else:
             item_price = int(item_price.text[:-3].replace("\u202f", "").replace(".", "").replace(",", ""))

        
        if fuzz.partial_ratio(item_title,item_name) < 55:
            print("The following listing does not match item name:", item_title)
        ...
        if not is_warehouse:
            ...
            shipping_cost = "Not specified"
            total_cost = item_price
            options_starting_at = "No more buying options"
            warehouse = "Yes"
        ...
        results_dict["Others Starting At"].append(options_starting_at)
        results_dict["Website"].append(amazon_name)
        results_dict["Warehouse"].append(warehouse)
    df =  pd.DataFrame.from_dict(results_dict)
    return df
    ...
    for item in SEARCH_ITEMS_LIST:
        ...
        for amazon_name, amazon_url in URLS.items():
            ...
            for url_dict in item_urls:
                ...
                result_df = url_dict["func"](url_dict["url"], amazon_url,  amazon_name, item["item_name"], url_dict["is_warehouse"])
        ...
        df_all = df_all.drop_duplicates(subset = ['Title', 'Item Price'], keep = 'first').reset_index(drop = True)
